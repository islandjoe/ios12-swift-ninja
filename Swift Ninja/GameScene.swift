...
//  Swift Ninja
//
//  Created by Arthur Kho on 24/07/2019.
...
class GameScene: SKScene {
  ...
  var score = 0 { didSet { gameScore.text = "Score: \(score)"} }
  ...
  func playSwooshSound() {
    ...
    let swooshSound  = SKAction.playSoundFileNamed(
        soundName,
  ...
  func createEnemy(forceBomb:ForceBomb = .random) {
    ...
    { // Bomb code here
      ...
      // 4
      if
      let path = Bundle.main.url(forResource: "sliceBombFuse", withExtension: "caf")
      {
        if
  ...
  func subtractLife() {
    ...
    if      lives == 2 { life = livesImages[0] }
    else if lives == 1 { life = livesImages[1] }
    else
    {
      life = livesImages[2]
  ...
  override func touchesMoved(_ touches: Set<UITouch>, with event: UIEvent?) {
    ...
    case var node as SKSpriteNode in nodesAtPoint
    {
      ...
      { // Destroy penguin
        ...
        // 7
        if
        let index = activeEnemies.firstIndex(of: node)
        {
          activeEnemies.remove(at: index )
        }
      ...
      { // Destroy bomb
        ...
        if
        let index = activeEnemies.firstIndex(of: bombContainer )
        {
          activeEnemies.remove(at: index )
        }
  ...
  func endGame(triggeredByBomb:Bool) {
    ...
    gameOver.scale(to: CGSize(width: 300, height: 240))
